export const dashboard = <i className="fa-solid fa-house"></i>
export const transactions = <i className="fa-solid fa-credit-card"></i>
export const categories = <i className="fa-solid fa-list"></i>
export const accounts = <i className="fa-solid fa-wallet"></i>
export const settings = <i className="fa-solid fa-gear"></i>
export const logout = <i className="fa-solid fa-right-from-bracket"></i>
export const trend = <i className="fa-solid fa-money-bill"></i>
export const expenses = <i className="fa-solid fa-money-bill-transfer"></i>
export const money = <i className="fa-solid fa-money-bill-wave"></i>
export const freelance = <i className="fa-solid fa-earth-americas"></i>
export const stocks = <i className="fa-solid fa-arrow-trend-up"></i>
export const bitcoin = <i className="fa-brands fa-bitcoin"></i>
export const royalty = <i className="fa-solid fa-crown"></i>
export const investment = <i className="fa-solid fa-sack-dollar"></i>
export const gift = <i className="fa-solid fa-gift"></i>
export const scholarship = <i className="fa-brands fa-google-scholar"></i>  
export const pension = <i className="fa-solid fa-wallet"></i>
export const savings = <i className="fa-solid fa-piggy-bank"></i> 
export const piggy = <i className="fa-solid fa-piggy-bank"></i> 
export const youtube = <i className="fa-brands fa-youtube"></i>
export const card = <i className="fa-regular fa-credit-card"></i>
export const users = <i className="fa-solid fa-user"></i>
export const dollar = <i className="fa-solid fa-dollar-sign"></i>
export const calender = <i className="fa-regular fa-calendar-days"></i>
export const comment = <i className="fa-solid fa-comment"></i>
export const plus = <i className="fa-solid fa-plus"></i>
export const trash = <i className="fa-solid fa-trash"></i>
export const signout = <i className="fa-solid fa-right-from-bracket"></i>
export const clothing = <i className="fa-solid fa-shirt"></i>
export const book = <i className="fa-solid fa-book"></i>
export const food = <i className="fa-solid fa-bowl-food"></i>
export const medical = <i className="fa-solid fa-notes-medical"></i>
export const tv = <i className="fa-solid fa-tv"></i>
export const circle = <i className="fa-solid fa-circle-dot"></i>
export const rupees = <i className="fa-solid fa-indian-rupee-sign"></i>